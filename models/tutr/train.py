...
def train(epoch, model, reg_criterion, cls_criterion, optimizer, train_dataloader, motion_modes):
    ...
    for i, (ped, neis, mask) in enumerate(train_dataloader):
        ...
        optimizer.zero_grad()
        pred_traj, scores, _ = model(ped_obs, neis_obs, motion_modes, mask, closest_mode_indices)
...
if __name__ == "__main__":
    ...
    for subset in datasets:
        ...
        args.hp_config = f'config/config_DLA/{subset}.py'
        ...
        motion_modes_file = f'./checkpoint/checkpoint_DLA/{args.dataset_name}_motion_modes.pkl'
        ...
        motion_modes = get_motion_modes(train_dataset, args.obs_len, args.pred_len, hp_config.n_clusters,
                                        ...
                                        motion_modes_file=motion_modes_file)
        ...
        test_loader = DataLoader(test_dataset, collate_fn=test_dataset.coll_fn, batch_size=hp_config.batch_size,
                                 shuffle=True, num_workers=args.num_works)
        ...
        for ep in range(hp_config.epoch):
            ...
            if min_fde + min_ade > ade + fde:
                ...
                min_ade = ade
                min_fde_epoch = ep
                torch.save(model.state_dict(), f'./checkpoint/checkpoint_DLA/{args.dataset_name}_best.pth')  # OK

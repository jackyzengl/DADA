...
def plot_trajectories(ax,
    ...
    for node in histories_dict:
        ...
        if aa >= 0.2:
            ...
            d3 = np.array([+0.1, +0.1])
            d4 = np.array([+0.1, -0.1])
            future = np.expand_dims(np.expand_dims(futures_dict[node], axis=0).repeat(1, axis=0), axis=0)
            f1 = np.expand_dims(np.expand_dims(futures_dict[node]+d1, axis=0).repeat(1, axis=0), axis=0)
            f2 = np.expand_dims(np.expand_dims(futures_dict[node]+d2, axis=0).repeat(1, axis=0), axis=0)
            f3 = np.expand_dims(np.expand_dims(futures_dict[node]+d3, axis=0).repeat(1, axis=0), axis=0)
            f4 = np.expand_dims(np.expand_dims(futures_dict[node]+d4, axis=0).repeat(1, axis=0), axis=0)
            predictions = np.concatenate((prediction_dict[node][:,:15,:,:], future, f1, f2, f3, f4), axis=1)
        else:
        ...
        predictions_ = predictions[batch_num]
        
        traj_data_reshape = predictions_.reshape(-1, 2)
        sns.kdeplot(traj_data_reshape[:, 0], traj_data_reshape[:, 1], 
                    ax=ax, shade=True, shade_lowest=False, cmap=cmap1[t%la])
        t += 1
    ...
    for node in histories_dict:
        ...
        ax.plot(history[:, 0], history[:, 1], linestyle='--', color=cmap2[t%la])
        future = np.vstack((history[-1], future))
        ax.plot(future[:, 0], future[:, 1], linestyle='-', color=cmap2[t%la])
        # path_effects=[pe.Stroke(linewidth=edge_width, foreground='g'), pe.Normal()]
        
        dx, dy = future[-1, 0] - future[-2, 0], future[-1, 1] - future[-2, 1]
        ax.arrow(future[-2, 0], future[-2, 1], dx, dy, head_width=0.2, fc=cmap2[t%la], ec=cmap2[t%la])

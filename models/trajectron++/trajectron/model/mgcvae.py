...
from model.tt_models import Discriminator_inlay
from model.tt_losses import gan_d_loss, gan_g_loss
...
class MultimodalGenerativeCVAE(object):
    def __init__(self,
        ...
        self.min_hl = self.hyperparams['minimum_history_length']  # 1
        self.max_hl = self.hyperparams['maximum_history_length']  # 7
    ...
    def create_node_models(self):
        ...
        self.add_submodule(self.node_type + '/discriminator_inlay',
                           model_if_absent=Discriminator_inlay(64, 128))
    ...
    def obtain_encoded_tensors(self,
        ...
        #########################################
        node_history = inputs  # [nums, 8, 6]
        node_present_state = inputs[:, -1]  # [nums, 6]
        node_pos = inputs[:, -1, 0:2]  # [nums, 2]
        node_vel = inputs[:, -1, 2:4]  # [nums, 2]

        node_history_st = inputs_st  # abs[nums, 8, 6]
        node_present_state_st = inputs_st[:, -1]
        node_pos_st = inputs_st[:, -1, 0:2]
        node_vel_st = inputs_st[:, -1, 2:4]

        n_s_t0 = node_present_state_st  # [nums, 6]

        initial_dynamics['pos'] = node_pos  # [nums, 2]
        initial_dynamics['vel'] = node_vel  # [nums, 2]
        ...
        # Encode Present #
        ##################
        node_present = node_present_state_st  # [bs, state_dim], [nums, 4]
    ...
    def discriminator_inlay(self, 
                            x):  # [256, 64]
        net_D = self.node_modules[self.node_type + '/discriminator_inlay']
        return net_D(x)  # [256, 64]
    ...
    def q_z_xy(self, mode, x, y_e) -> torch.Tensor:
        ...
        return self.latent.dist_from_h(to_latent(h),  # [256, 25]
                                       mode)
    ...
    def train_loss(self,
                   inputs,  # [bs=256, 8, 6]
                   inputs_st,  # [bs=256, 8, 6]
                   first_history_indices,
                   labels,  # [bs=256, 12, 2]
                   labels_st,  # [bs=256, 12, 2]
                   neighbors,
                   neighbors_edge_value,
                   robot,
                   map,
    ...
    def train_loss_D(self,
                     inputs_s,  # [bs=256, 8, 6]
                     inputs_st_s,  # [bs=256, 8, 6]
                     first_history_indices_s,
                     labels_s,  # [bs=256, 12, 2]
                     labels_st_s,  # [bs=256, 12, 2]
                     neighbors_s,
                     neighbors_edge_value_s,
                     robot_s, map_s,
                     inputs_t,  # [bs=256, 8, 6]
                     inputs_st_t,  # [bs=256, 8, 6]
                     first_history_indices_t,
                     labels_t,  # [bs=256, 12, 2]
                     labels_st_t,  # [bs=256, 12, 2]
                     neighbors_t,
                     neighbors_edge_value_t,
                     robot_t, map_t,
                     prediction_horizon):
        mode = ModeKeys.TRAIN
        x_s, x_nr_s, y_e_s, y_r_s, y_s, n_s_t0_s = self.obtain_encoded_tensors(
            mode=mode,
            inputs=inputs_s,
            inputs_st=inputs_st_s,
            labels=labels_s,
            labels_st=labels_st_s,
            first_history_indices=first_history_indices_s,
            neighbors=neighbors_s,
            neighbors_edge_value=neighbors_edge_value_s,
            robot=robot_s, map=map_s)
        x_t, x_nr_t, y_e_t, y_r_t, y_t, n_s_t0_t = self.obtain_encoded_tensors(
            mode=mode,
            inputs=inputs_t,
            inputs_st=inputs_st_t,
            labels=labels_t,
            labels_st=labels_st_t,
            first_history_indices=first_history_indices_t,
            neighbors=neighbors_t,
            neighbors_edge_value=neighbors_edge_value_t,
            robot=robot_t, map=map_t)
        # x_s.shape, [256, 64]

        feature_s = self.discriminator_inlay(x_s.detach())
        feature_t = self.discriminator_inlay(x_t.detach())
        D_loss_real, D_loss_fake = gan_d_loss(
            scores_real=feature_t,
            scores_fake=feature_s,
            mode='mse')
        D_loss = D_loss_real + D_loss_fake
        
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'D_inlay_loss_real'),
                                       D_loss_real,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'D_inlay_loss_fake'),
                                       D_loss_fake,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'D_inlay_loss'),
                                       D_loss,
                                       self.curr_iter)
        return D_loss
    
    def train_loss_G(self,
                     inputs_s,  # [bs=256, 8, 6]
                     inputs_st_s,  # [bs=256, 8, 6]
                     first_history_indices_s,
                     labels_s,  # [bs=256, 12, 2]
        ...
        feature_s = self.discriminator_inlay(x_s)
        G_loss = gan_g_loss(
            scores_fake=feature_s,
            mode='mse'
        )
        ...
        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'G_inlay_loss'),
                                       G_loss,
                                       self.curr_iter)
            if self.hyperparams['log_histograms']:
                self.latent.summarize_for_tensorboard(self.log_writer, str(self.node_type), self.curr_iter)
    ...
    def train_loss_trajectron(self,
                              inputs_s,  # [bs=256, 8, 6]
                              inputs_st_s,  # [bs=256, 8, 6]
                              first_history_indices_s,
                              labels_s,  # [bs=256, 12, 2]
        ...
        z, kl = self.encoder(mode, x_s, y_e_s)
        log_p_y_xz = self.decoder(mode, x_s, x_nr_s, y_s, y_r_s, n_s_t0_s, z,
                                  labels_s,  # Loss is calculated on unstandardized label
                                  prediction_horizon,
                                  self.hyperparams['k'])
        ...
        if self.log_writer is not None:
            ...
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'log_likelihood'),
                                       log_likelihood,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'trajectron_loss'),
                                       trajectron_loss,
                                       self.curr_iter)
            if self.hyperparams['log_histograms']:
                self.latent.summarize_for_tensorboard(self.log_writer, str(self.node_type), self.curr_iter)
                
        return trajectron_loss

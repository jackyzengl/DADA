...
def seq_collate(data):
    ...
    whole_traj = torch.cat(whole_seq_list, dim=0).permute(2, 0, 1)

    obs_traj_rel = torch.cat(obs_seq_rel_list, dim=0).permute(2, 0, 1)
    pred_traj_rel = torch.cat(pred_seq_rel_list, dim=0).permute(2, 0, 1)
    whole_traj_rel = torch.cat(whole_seq_rel_list, dim=0).permute(2, 0, 1)
...
class TrajectoryDataset(Dataset):
    ...
    def __init__(self,
                 data_dir,
                 obs_len=8, pred_len=12, whole_len=20,
        ...
        for path in all_files:
            data = read_file(path, delim)  # [lines, 4]
        ...
        self.whole_traj = torch.from_numpy(seq_list[:, :, :]).type(
            torch.float
        )
        ...
        self.whole_traj_rel = torch.from_numpy(seq_list_rel[:, :, :]).type(
            torch.float
        )
